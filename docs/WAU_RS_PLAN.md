...
- [x] **4e** — **GitHub** provider behind `github` feature: release-asset mode (asset regex) + git-ref tip mode (zipball); mocked-HTTP unit tests
- [x] **4f** — `resolve/` module (manifest filtering layer); `wau` feature passthroughs per §2.2; all quality gates green; update `WAU_RS_PLAN.md`
...
- [x] Ensure `main` does not carry migration-era scaffolding (temporary notes, "TODO diaries", stale docs)
- [x] Ensure docs and `examples/` reflect current schema versions
- [x] `wau init` auto-generates manifest (`manifest.toml`) and lock (`lock.toml`) files
...
| 2026-04-28 | Phase 4f complete: `libwau::resolve` directory module — `plan()` filters manifest by flavor and lock state, returns `ResolutionPlan { to_install, skipped }`; 9 unit tests; `wau` feature passthroughs (`local`/`curseforge`/`wowinterface`/`github`) forward to `libwau` features per §2.2; `app::sync` uses `resolve::plan`; all quality gates green with `--all-features` and `--no-default-features`                                                            |
...
| 2026-04-29 | Phase 6 complete: removed migration-era comment and stale `#![allow(dead_code)]` from `wau/src/config/mod.rs`; updated `examples/config.toml` (TBD markers resolved, GitHub provider section added); updated `examples/lock.toml` (lock-file naming clarified); updated `examples/cli.md` (resolved TBDs, split implemented vs deferred commands); rewrote `README.md` (reflects implemented feature set, providers, config, and workspace layout); tagged `v0.1.0` |
| 2026-04-29 | Phase 6 final item: `wau init` implemented — `Manifest::new()` + `Default` + `manifest::save()`; `--tag`/`--manifest`/`--force` flags; `InitSettings` in settings resolution; `init()` in `app::run`; `print_init_header/created/skipped` output helpers; 9 new tests across `libwau` and `wau`; `examples/cli.md` updated with Init section; Phase 6 fully complete                                                                                                |
